...
#Modell definieren
model = Sequential()
model.add(Dense(units = 182, input_shape=(4,), activation='relu'))   #Eingangschicht
model.add(Dense(units = 182, activation='relu'))                #Mittelschichten
...
def create_model(trial):
    model = Sequential()
    model.add(Dense(units=trial.suggest_int('num_neurons', 50, 200), input_shape=(4,), activation='relu'))  # Eingangsschicht
...
history = model.fit(train_inputs.reshape(-1,4), train_outputs.reshape(-1,1), epochs=5, batch_size=1, callbacks=callbacks_list, validation_data=(validate_inputs,validate_outputs), verbose=2)
