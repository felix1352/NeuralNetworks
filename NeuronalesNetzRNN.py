...
def generiere_trainingsdaten(startwerte):
    ...
    for x0_train in startwerte:
        ...
        current_output_train = berechne_winkel(x0_train)
...
plt.plot(time_plot,output_test,'r', label='Real output')

output_test = np.reshape(output_test,(1,output_test.shape[0],1))

#Model erstellen
model = Sequential()
model.add(SimpleRNN(64, input_shape=(trainX.shape[1],trainX.shape[2]),return_sequences=True))
model.add(Dense(1))
...
#Modell trainieren
history = model.fit(trainX, output_train, epochs=50, batch_size=1, validation_split=0.2)
...
plt.plot(time_plot,testPredict[0,:,0], label='Predicted output')
